...
sub AllvsAll_uSearch
{
	...
	system("perl $Program_dir/usearch.pl $id $threads");
	my @comb = glob("*.comb");
	unlink @comb;
	system("touch AllProteinList.all.cluster");

	open(USEARCH,">>AllProteinList.all.cluster") or die print "Could not create AllProteinList.all.cluster\n";
	my @usearch = glob("*.usearch");
	foreach my $file (@usearch){
		#print $_ . "\n\n";
		if (open my $in, '<', $file){
			while (my $line = <$in>){
				print USEARCH $line;
		...
		else{warn "Could not open '$file' for reading\n";}
		}
	close(USEARCH);

	#system("rm *.usearch");
	my @refine = glob("*.refine*");
	unlink @refine;
...
sub SortUSearch
{
	...
	#Using the standard PERL Hash sort does not work for extremely large files
	system("sort --parallel=$threads -uo AllProteinList.all.cluster.sorted AllProteinList.all.cluster");
...
sub ParseBlast_USearch
{
	...
	$Program_dir = "$FindBin::RealBin/Support";
	system("perl $Program_dir/reFFNer_v3.pl ./PanGenome.usearch $ffn_dir $threads");
	chdir($home);
